package OracleAlertLog::ParseOptions;
...
#      PACKAGE: OracleAlertLog::ParseOptions
#
#  DESCRIPTION: ParseOptions for OracleAlertLog
#
#        FILES: ---
#         BUGS: ---
#        NOTES: ---
...
sub parse {
	...
	# minutes,hours,days
	#
	&error($caller,'$Options{minutes|hours|days} must be defined') if not ( $Options{'minutes'} or $Options{'hours'} or $Options{'days'} );

    if($Options{'minutes'}){ 
		$Options{'time'} = $Options{'minutes'};
	    $Options{'time_unit'} = 'MINUTE';	
	}
    if($Options{'hours'}){ 
		$Options{'time'} = $Options{'hours'};
	    $Options{'time_unit'} = 'HOUR';	
	}
    if($Options{'days'}){ 
		$Options{'time'} = $Options{'days'};
	    $Options{'time_unit'} = 'DAY';	
	}
...
OracleAlertLog::ParseOptions - ParseOptions for OracleAlertLog 

=head1 SYNOPSIS

use OracleAlertLog::ParseOptions;

my $object = OracleAlertLog::ParseOptions->new();
