package OracleRmanBackup::ParseOptions;
...
#      PACKAGE: OracleRmanBackup::ParseOptions
#
#  DESCRIPTION: ParseOptions for OracleRmanBackup
#
#        FILES: ---
#         BUGS: ---
#        NOTES: ---
...
sub parse {
	...
	# interval
	#
	&error($caller,'$Options{interval} in days must be defined') if not ( $Options{'I'} or $Options{'interval'} ); 
	if ($Options{'I'}) { $Options{'interval'} = $Options{'I'} };
	if ($Options{'interval'}) { $Options{'I'} = $Options{'interval'} };
	&error($caller,'$Options{interval} must be 100< days') if (  $Options{interval} >= 100 ); 
	&verbose($caller,'$Options{interval} = ' . $Options{'interval'}  ) if ( $Options{'v'} or $Options{'verbose'} ); 
...
OracleRmanBackup::ParseOptions - ParseOptions for OracleRmanBackup 

=head1 SYNOPSIS

use OracleRmanBackup::ParseOptions;

my $object = OracleRmanBackup::ParseOptions->new();
